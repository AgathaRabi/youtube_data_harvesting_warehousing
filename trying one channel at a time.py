...
def get_channel_info(channel_id):
    ...
    for information in channel_info_response[
        'items']:  ### here you are getting only the details you need of the channel.
...
def get_channel_video_id(current_channel_id):
    ...
    get_video_ids = youtube_access.playlistItems().list(part='snippet',
                                                        playlistId=upload_id_vd_id).execute()  # , maxResults = 5)
    ...
    for each_video_id in obt_video_ids:
        ...
        for item in get_video_details["items"]:
            ...
            video_meta_data = dict(Channel_Name=item['snippet']['channelTitle'],
                                   ...
                                   Description=item['snippet']['description'] if 'description' in item[
                                       'snippet'] else None,
        ...
        for every_video_id in total_video_ids:
            ...
            for comment_detail in to_get_comment_details['items']:
                ...
                comment_meta_data = dict(Comment_Gvn_Id=comment_detail['snippet']['topLevelComment']['id'],
                                         ...
                                         Comment_Text=comment_detail['snippet']['topLevelComment']['snippet'][
                                             'textDisplay'],
                                         Comment_Author=comment_detail['snippet']['topLevelComment']['snippet'][
                                             'authorDisplayName'],
                                         Comment_Published_Date=comment_detail['snippet']['topLevelComment']['snippet'][
                                             'publishedAt'])
